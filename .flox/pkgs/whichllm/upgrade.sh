...
latest_tag=$(curl -sfL \
  "https://api.github.com/repos/Andyyyy64/whichllm/releases/latest" \
  | jq -r '.tag_name')

if [ -z "$latest_tag" ] || [ "$latest_tag" = "null" ]; then
  echo "ERROR: failed to fetch latest whichllm tag (rate limited?)" >&2
  exit 1
fi
...
echo "Updating whichllm to $latest_tag"
...
uv lock --upgrade-package whichllm
popd > /dev/null
