...
import (
	...
	"flox.dev/flox-ai/internal/audit/findings"
	"flox.dev/flox-ai/internal/audit/score"
	"flox.dev/flox-ai/internal/audit/tools"
)
...
type Options struct {
	...
	WithBehavioral bool
	Findings       bool
	IncludeRaw     bool // retain each tool's raw stdout on Result.RawByTool
}
...
type Result struct {
	...
	RawByTool map[string]string  `json:"rawByTool,omitempty"`
}
...
func Run(opts Options) (Result, error) {
	...
	// Raw per-tool output for the report's raw pane. runTool returns cached
	// output (outCache / claudelint cache), so this triggers no extra runs.
	if opts.IncludeRaw && !dry() {
		r.RawByTool = map[string]string{}
		for _, t := range sel {
			r.RawByTool[t.Name] = string(rn.runTool(t))
		}
		for _, t := range tools.SecurityTools(kind) {
			r.RawByTool[t.Name] = string(rn.runTool(t))
		}
	...
	if opts.IncludeRaw && dry() {
		// Dry-run: synthesize a stub so tests and offline use see a value.
		r.RawByTool = map[string]string{}
		for _, t := range sel {
			r.RawByTool[t.Name] = "(dry-run) would run " + t.Bin
		}
	}
