...
in
stdenv.mkDerivation {
  pname = "claude-code-plugin-official-rust-analyzer-lsp";
  ...
  installPhase = ''
    ...
    PLUGIN_DIR="$out/share/claude-code/plugins/rust-analyzer-lsp"
    mkdir -p "$PLUGIN_DIR/.claude-plugin"

    cp "$src/plugins/rust-analyzer-lsp/LICENSE" \
       "$src/plugins/rust-analyzer-lsp/README.md" \
       "$PLUGIN_DIR/"

    entry=$(jq '.plugins[] | select(.name == "rust-analyzer-lsp")' \
