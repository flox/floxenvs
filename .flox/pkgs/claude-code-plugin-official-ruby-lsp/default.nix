{ stdenv, lib, fetchFromGitHub, jq, rubyPackages, flox-agent-layout }:
...
in
stdenv.mkDerivation {
  pname = "claude-code-plugin-official-ruby-lsp";
  ...
  postInstall = ''
    flox-agent-layout --plugin ruby-lsp --share "$out/share"
  '';
