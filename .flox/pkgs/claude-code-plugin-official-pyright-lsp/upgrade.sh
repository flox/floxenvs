...
if [ "$current_version" = "$new_version" ]; then echo "Already up to date"; exit 0; fi
echo "Updating to $new_version"
sha256_base32=$(nix-prefetch-url --unpack "https://github.com/$OWNER/$REPO/archive/$rev.tar.gz")
sri_hash=$(nix --extra-experimental-features nix-command hash convert --hash-algo sha256 --to sri "$sha256_base32")
jq -n --arg v "$new_version" --arg r "$rev" --arg h "$sri_hash" '{version: $v, rev: $r, srcHash: $h}' > "$HASHES_FILE"
