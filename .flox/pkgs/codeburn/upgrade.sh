...
if [ "$current_version" = "$latest_version" ]; then
  echo "Forcing regeneration of codeburn at $current_version"
else
  echo "Updating codeburn from $current_version to $latest_version"
fi
...
prefetch_log=$(mktemp)
flox build codeburn > "$prefetch_log" 2>&1 || true
