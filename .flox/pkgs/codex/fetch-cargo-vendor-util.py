...
def download_file_with_checksum(session: requests.Session, url: str, destination_path: Path) -> str:
    # FLOX PATCH: crates.io returns 403 on bursts of concurrent requests from
    # the same egress IP. The upstream retry policy only covers 5xx. Handle
    # 403 / 429 here with a short linear backoff so a single rate-limited
    # request doesn't abort the whole build.
        ...
        try:
            with session.get(url, stream=True) as response:
                if response.status_code in (403, 429):
                    last_err = f"status {response.status_code} (attempt {attempt + 1})"
                    _time.sleep(1 + attempt * 0.5)
                    continue
                if not response.ok:
            ...
            last_err = str(e)
            _time.sleep(1 + attempt * 0.5)
    raise Exception(f"Failed to fetch file from {url}. Last error: {last_err}")
...
def create_vendor_staging(lockfile_path: Path, out_dir: Path) -> None:
    ...
    # FLOX PATCH: lower parallelism from 5 → 2 to reduce crates.io 403s under
    # rate-limiting. Combined with the per-request 403/429 retry in
    # download_file_with_checksum, this is enough to get through a full build.
