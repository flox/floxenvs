...
stdenvNoCC.mkDerivation {
  pname = "claude-code-plugin-official-code-review";
  ...
  installPhase = ''
    runHook preInstall
    PLUGIN_DIR="$out/share/claude-code/plugins/code-review"
    mkdir -p "$PLUGIN_DIR"
    cp -r "$src/plugins/code-review/." "$PLUGIN_DIR/"
    runHook postInstall
  '';

  postInstall = ''
    flox-agent-layout --plugin code-review --share "$out/share"
  '';
